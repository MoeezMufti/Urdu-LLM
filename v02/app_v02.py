...
client = OpenAI()  # OpenAI() will now pick up your API key

def require_api_key():
    if not os.getenv("OPENAI_API_KEY"):
        st.error("⚠️ OPENAI_API_KEY not set. Create a .env file with your key or export it in your shell.")
        st.stop()
...
def ai_summarize(text: str) -> str:
    """Use GPT to summarize Urdu text"""
    prompt = f"مندرجہ ذیل اردو متن کا مختصر اور جامع خلاصہ دیں:\n\n{text}"
    response = client.chat.completions.create(
        model="gpt-4o-mini",   # lightweight & cheap; change to gpt-4o for higher quality
...
def ai_qa(text: str, question: str) -> str:
    """Use GPT for Q&A based on given text"""
    prompt = f"""
آپ کو ایک اردو متن دیا گیا ہے۔ براہ کرم اس متن کی بنیاد پر نیچے دیے گئے سوال کا واضح اور درست جواب دیں۔
---
متن:
{text}
    ...
    with col2:
        ...
        if st.button("📄 خلاصہ بنائیں", use_container_width=True):
            if text_input.strip():
                ...
                summary = ai_summarize(text_input)
        ...
        with col1:
            if st.button("🔍 جواب دیں"):
                if question.strip() and text_input.strip():
                    ...
                    answer = ai_qa(text_input, question)
